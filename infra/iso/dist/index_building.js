...
let sketch = function(p) {
  ...
  const mag = 18;
  const xu = [1 * mag, 0.1 * mag]; // X Unit
  const yu = [0.15 * mag, 0.75 * mag]; // Y Unit
  const zu = [0.05 * mag, -0.05 * mag]; // Y Unit

  const decorEnabled = false;
  const shadingEnabled = true;
  const strokeEnabled = false;

  const palette = get_palette();
  const generator = new Apparatus(12, 25, {
    simple: false,
  ...
  p.setup = function() {
    ...
    //if (strokeEnabled) p.stroke(0, 50);
    else p.noStroke();
  };
  ...
  function createGrid(box) {
    ...
    const apparatus = createApparatus(cell_w, cell_h).map(app => ({
      ...app,
      z1: 10 + Math.floor(Math.random() * 20)
    }));
